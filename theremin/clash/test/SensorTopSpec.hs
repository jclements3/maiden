...
-- | The settled stage-1 measurements, ignoring the pre-fill zeros.
...
tests = testGroup "sensor chain end to end"
  ...
  , testCase "the two axes measure independently" $ do
      ...
      let outs = runTop (oscStream 16 24 (clocksFor 24))
          pitchVs = P.dropWhile (== 0) (P.map (payload . soPitchPeriodStage1) outs)
          volVs   = P.dropWhile (== 0) (P.map (payload . soVolPeriodStage1) outs)
  ...
  , testCase "stage 2 converges towards stage 1" $ do
      -- The IIR has unity DC gain but truncates, so it settles at or just
      -- below stage 1 -- never above, and never at zero once running.
      ...
      assertBool "stage 2 does not exceed stage 1" (s2 <= s1 * 4)
  ]
