...
- [ ] Write `software/maiden/geo.py`: `ecef_from_lla`, `enu_from_lla`
...
- [ ] Write `config/field/rcrc.yaml`: site block, `field_origin` = Station
...
- [ ] Write `software/tests/test_geo.py` — the three tests specified in
      lesson 01 §Build: known-answer (1° longitude oracle from the
      formula), flat-earth agreement ≤ 5 mm within 300 m, origin/symmetry.
- [ ] REPL sanity check: point 150 m due north of Station A returns
      ENU ≈ (0, 150, 0) within centimeters.
- [ ] Explore 1 (drop the `(1-e²)`, watch ~135 m of Up error appear) —
      then put it back.
