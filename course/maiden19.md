...
- [ ] Implement `software/maiden/track/candidates.py`: `SkyModel`
      ...
      contrast, coherence)`.
- [ ] Fill `coherence` from a one-frame memory of candidate centroids
      (nearest-neighbor displacement); keep it simple — maiden20 replaces
      it with real gating.
- [ ] Implement `software/maiden/track/metrics.py`: recall vs. target
...
- [ ] Write `software/tests/test_candidates.py`: known blob proposed,
      ...
      ≥ 0.95 at ≥ 6 px.
- [ ] Sun run: re-render with the sun on the path, plot the
      recall-vs-time trace, and log dip depth and width to
      `results/VT-15/twin/`.
- [ ] Time the candidate stage on 1080p30 input; note frames/sec in the
