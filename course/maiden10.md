...
- [ ] Implement `sportsman(imp, seed)` in `twin/model.py`: takeoff leg,
...
- [ ] Emit `MANEUVER_START`/`MANEUVER_END` Events with
      `data={"maneuver": name}` at every boundary.
- [ ] Wire the `Imperfections` knobs so they demonstrably deform the path
      (loop_ovality, roll_drift_deg, alt_mismatch_m, center_offset_m).
- [ ] Complete `software/tests/test_twin_model.py` per the lesson's
      ...
      seed determinism.
- [ ] Write `python -m maiden.twin.plot` (3D path + plan view, boundaries
      marked from Events) and look at it.
- [ ] Explore 1 at minimum: overlay `ovality=0.15` against the ideal loop
